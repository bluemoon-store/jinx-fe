...
export const DashboardDropsLoadMoreFooter: FunctionComponent<{
  shown: number
  total: number
  onLoadMore: () => void
  canLoadMore: boolean
  ...
  return (
    <div className="font-commissioner text-foreground relative flex w-full flex-col items-center justify-center gap-5 pt-2 text-center text-base sm:gap-5 dark:text-white">
      ...
      {canLoadMore ? (
        <button
          type="button"
          onClick={onLoadMore}
          className="border-border-subtle bg-card text-foreground dark:border-darkslateblue rounded-num-30 flex items-center justify-center border-[1.5px] border-solid px-6 py-2.5 shadow-[0px_15px_15px_rgba(0,0,0,0.01)] transition-opacity dark:bg-gray-100 dark:text-white"
        >
